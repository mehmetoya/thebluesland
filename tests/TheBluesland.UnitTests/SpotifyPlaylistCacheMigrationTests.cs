...
public sealed class SpotifyPlaylistCacheMigrationTests : IAsyncLifetime
{
    ...
    public async Task Migration_creates_spotify_playlist_cache_table_with_expected_columns()
    {
        ...
        actualColumns["era_bucket_counts"].DataType.ShouldBe("ARRAY");
        actualColumns["era_bucket_counts"].IsNullable.ShouldBeTrue();
        actualColumns["follower_count"].DataType.ShouldBe("integer");
        actualColumns["follower_count"].IsNullable.ShouldBeTrue();
