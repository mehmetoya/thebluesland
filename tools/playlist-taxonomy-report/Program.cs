...
var eraCounts = published.SelectMany(p => p.Eras).GroupBy(x => x).OrderByDescending(g => g.Count());
...
var allText = published
    .OrderBy(p => p.Slug)
    .Select(p =>
        $"--- {p.Slug} | moods=[{string.Join(",", p.Moods)}] eras=[{string.Join(",", p.Eras)}] occasions=[{string.Join(",", p.Occasions)}] ---\n" +
        $"SUMMARY: {p.Summary}\n" +
        $"NOTE: {p.CuratorNote}\n");
